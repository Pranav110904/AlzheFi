...
// ─── Theme (identical to patient side) ───────────────────────────────────────
const YELLOW = '#E3F73F';
const DARK   = '#1F2937';
const BG     = '#F4F1EC';
const MUTED  = '#9CA3AF';
const BORDER = '#E5E0D8';
const WHITE  = '#FFFFFF';
const TEAL   = '#0D9488'; // caregiver accent
...
// ─── Helpers (copied from patient screen) ────────────────────────────────────
...
const IMPORTANCE_CONFIG: Record<Importance, {
  ...
  low:    { label: 'Low',    icon: 'bell-outline',      color: '#6B7280', bg: '#F9FAFB', border: '#E5E7EB', desc: 'Nice to remember, not critical' },
  medium: { label: 'Medium', icon: 'bell-ring-outline', color: '#D97706', bg: '#FFFBEB', border: '#FDE68A', desc: 'Should not be missed' },
  high:   { label: 'High',   icon: 'bell-alert',        color: '#DC2626', bg: '#FEF2F2', border: '#FECACA', desc: 'Critical — must not be missed' },
};
        ...
        {cells.map((day, idx) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.75}
            >
              <Text style={[cal.cellText, isTod && !isSel && cal.cellTextToday, isSel && cal.cellTextSelected, isPast && cal.cellTextPast]}>
                {day}
              </Text>
            </TouchableOpacity>
          );
  ...
  return (
    <View style={tp.wrapper}>
      ...
      <View style={tp.preview}>
        ...
        <Text style={tp.previewSub}>{hour < 12 ? 'Morning' : hour < 17 ? 'Afternoon' : hour < 21 ? 'Evening' : 'Night'}</Text>
      </View>
    </View>
  );
}
...
function ImportanceBadge({ importance }: { importance: Importance }) {
  ...
  return (
    <View style={[styles.importanceBadge, { backgroundColor: cfg.bg, borderColor: cfg.border }]}>
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={[modal.sheetWrapper, { position: 'absolute', bottom: 0, left: 0, right: 0 }]}>
        <View style={[modal.sheet, { paddingBottom: Platform.OS === 'ios' ? 44 : 28 }]}>
          ...
          <Text style={styles.selectorSubtitle}>Choose who you're adding a reminder for</Text>
...
// ─── Add Event Modal (caregiver version) ─────────────────────────────────────
function AddEventModal({ visible, onClose, onAdd, patientName }: {
  visible: boolean;
  onClose: () => void;
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onAdd({ title: title.trim(), description: desc.trim() || undefined, datetime: dt.toISOString(), importance });
      reset(); onClose();
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={modal.sheetWrapper}>
        <View style={modal.sheet}>
          ...
          <View style={styles.forPatientBanner}>
            ...
            <Text style={styles.forPatientText}>For <Text style={{ fontFamily: 'Coolvetica-Bold' }}>{patientName}</Text></Text>
          </View>

          {/* Step dots */}
          <View style={modal.stepRow}>
            {[1, 2, 3, 4].map(n => (
              <View key={n} style={[modal.stepDot, stepNums[step] >= n && modal.stepDotActive, stepNums[step] === n && modal.stepDotCurrent]} />
            ))}
          </View>
          ...
          <View style={modal.titleRow}>
            <View>
              <Text style={modal.stepLabel}>Step {stepNums[step]} of 4</Text>
          ...
          {step === 'importance' && (
            <View>
              ...
              <View style={modal.importanceGrid}>
                {(['low', 'medium', 'high'] as Importance[]).map(level => {
                  ...
                  return (
                    <TouchableOpacity
                      key={level}
                      style={[modal.importanceCard, { borderColor: cfg.border, backgroundColor: cfg.bg }, isSelected && { borderColor: cfg.color, borderWidth: 2.5 }]}
              ...
              <View style={modal.navBtns}>
                ...
                <TouchableOpacity style={modal.nextBtn} onPress={() => setStep('date')}>
                  <Text style={modal.nextBtnText}>Next</Text>
                  <Icon name="arrow-right" size={18} color={DARK} />
                </TouchableOpacity>
              </View>
          ...
          {step === 'date' && (
            <View>
              ...
              <View style={modal.navBtns}>
                ...
                <TouchableOpacity style={[modal.nextBtn, !selDate && modal.nextBtnDisabled]} onPress={() => setStep('time')} disabled={!selDate}>
                  <Text style={modal.nextBtnText}>Next</Text>
                  <Icon name="arrow-right" size={18} color={DARK} />
                </TouchableOpacity>
              </View>
          ...
          {step === 'time' && (
            <View>
              <View style={modal.summaryCard}>
                <View style={[modal.summaryIcon, { backgroundColor: IMPORTANCE_CONFIG[importance].color + '22' }]}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={modal.summaryMeta}>
                    ...
                    <View style={[modal.summaryImportancePill, { backgroundColor: IMPORTANCE_CONFIG[importance].bg, borderColor: IMPORTANCE_CONFIG[importance].border }]}>
                      <Text style={[modal.summaryImportanceText, { color: IMPORTANCE_CONFIG[importance].color }]}>{IMPORTANCE_CONFIG[importance].label}</Text>
                    </View>
                  </View>
                </View>
              </View>
...
function EventCard({ event, onDelete }: { event: Event; onDelete: (id: string) => void }) {
  ...
  return (
    <View style={[styles.card, past && styles.cardPast]}>
      <View style={[styles.cardStrip, { backgroundColor: past ? MUTED : IMPORTANCE_CONFIG[event.importance ?? 'medium'].color }]} />
      ...
      <View style={styles.cardInfo}>
        ...
        <Text style={[styles.cardTitle, past && styles.cardTitlePast]} numberOfLines={2}>{event.title}</Text>
        {event.description ? <Text style={styles.cardDesc} numberOfLines={1}>{event.description}</Text> : null}
        <View style={styles.cardTimeRow}>
          ...
          {until && <View style={styles.countdownPill}><Text style={styles.countdownText}>{until}</Text></View>}
          {past && <View style={styles.pastPill}><Text style={styles.pastPillText}>Done</Text></View>}
        </View>
      </View>
      <TouchableOpacity style={styles.deleteBtn} onPress={() => onDelete(event._id)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
...
export default function ReminderScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPatients(data);
        if (data.length > 0) setSelectedPatient(data[0]); // auto-select first
      } catch (e) {
  ...
  const fetchEvents = async (patient = selectedPatient, filter = activeFilter) => {
    if (!patient) return;
    try {
      setLoadingEvents(true);
      const patientId = patient._id ?? patient.id;
      const data = await apiService.getPatientEvents(patientId);

      const now = Date.now();
      const filtered = filter === 'upcoming'
        ? data.filter((e: Event) => new Date(e.datetime).getTime() > now)
        : data;

      setEvents(filtered);
    } catch (e) {
      console.log('Fetch patient events error:', e);
    } finally {
      setLoadingEvents(false);
    }
  };

  // ── Load events when patient or filter changes ──
  useEffect(() => {
    if (!selectedPatient) return;
    fetchEvents(selectedPatient, activeFilter);
...
const handleAdd = async (data: Partial<Event>) => {
  ...
  try {
    ...
    const created = await apiService.createPatientEvent(patientId, data);

    if (!created?._id) {
      throw new Error("Event not created");
    }

    // ✅ only fetch, no notification
    await fetchEvents(selectedPatient, activeFilter);
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Remove Reminder', `Remove this reminder for ${selectedPatient?.name}?`, [
      { text: 'Keep', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await apiService.deletePatientEvent(id);
  ...
  const scheduleNotification = async (event: Event) => {
    try {
      ...
      const notifId = `reminder-${event._id}`;
      await notifee.cancelNotification(notifId);
      await notifee.createTriggerNotification(
        { id: notifId, title: event.title || 'Reminder', body: event.description || 'You have a reminder', android: { channelId: 'default', smallIcon: 'ic_launcher', pressAction: { id: 'default' } } },
        { type: 0, timestamp: triggerDate.getTime() }
      );
    } catch (err) {
  ...
  // ── Loading state (waiting for patients) ──
  if (loadingPatients) {
    return (
    ...
    return (
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>Link a patient from your profile to manage their reminders</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          style={styles.addBtn}
          onPress={() => selectedPatient && setModalVisible(true)}
          activeOpacity={0.85}
      ...
      {/* ── Patient Selector ── */}
      <TouchableOpacity style={styles.patientSelector} onPress={() => setSelectorVisible(true)} activeOpacity={0.85}>
        <View style={styles.patientSelectorLeft}>
          {selectedPatient && <PatientAvatar name={selectedPatient.name} size={36} />}
          <View>
            <Text style={styles.patientSelectorLabel}>Viewing reminders for</Text>
            <Text style={styles.patientSelectorName}>{selectedPatient?.name ?? 'Select patient'}</Text>
          </View>
        </View>
      ...
      {loadingEvents ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>Tap Add to set a reminder for {selectedPatient?.name}</Text>
            ...
            <View key={dayLabel}>
              <View style={styles.dayHeader}>
                <View style={[styles.dayBadge, dayLabel === 'Today' && styles.dayBadgeToday]}>
                  <Icon name={dayLabel === 'Today' ? 'calendar-today' : 'calendar-outline'} size={12} color={dayLabel === 'Today' ? DARK : MUTED} />
                  <Text style={[styles.dayLabel, dayLabel === 'Today' && styles.dayLabelToday]}>{dayLabel}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  headerEyebrow: { fontSize: 13, fontFamily: 'Coolvetica-Regular', color: MUTED, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 },
  ...
  addBtnText: { fontSize: 15, fontFamily: 'Coolvetica-Bold', color: DARK },
  ...
  patientSelectorLabel: { fontSize: 11, fontFamily: 'Coolvetica-Regular', color: MUTED, textTransform: 'uppercase', letterSpacing: 1 },
  patientSelectorName: { fontSize: 17, fontFamily: 'Coolvetica-Bold', color: DARK },
  switchBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: TEAL + '18', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: TEAL + '44' },
  switchBadgeText: { fontSize: 12, fontFamily: 'Coolvetica-Bold', color: TEAL },

  // ── Avatar ──
  avatar: { backgroundColor: TEAL + '22', borderWidth: 1.5, borderColor: TEAL + '55', justifyContent: 'center', alignItems: 'center' },
  avatarText: { fontFamily: 'Coolvetica-Bold', color: TEAL },

  // ── Patient list in selector ──
  ...
  patientRow: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: WHITE, borderRadius: 16, borderWidth: 1.5, borderColor: BORDER, paddingHorizontal: 16, paddingVertical: 14, marginBottom: 10 },
  ...
  selectedCheck: { width: 26, height: 26, borderRadius: 13, backgroundColor: TEAL, justifyContent: 'center', alignItems: 'center' },
  ...
  statChipText: { fontSize: 13, fontFamily: 'Coolvetica-Bold', color: DARK },
  ...
  filterTabText: { fontSize: 13, fontFamily: 'Coolvetica-Bold', color: MUTED },
  ...
  dayLabel: { fontSize: 13, fontFamily: 'Coolvetica-Bold', color: MUTED },
  ...
  cardEyebrow: { fontSize: 10, fontFamily: 'Coolvetica-Bold', color: MUTED, letterSpacing: 1.5 },
  cardTitle: { fontSize: 17, fontFamily: 'Coolvetica-Bold', color: DARK, marginBottom: 3, lineHeight: 22 },
  cardTitlePast: { color: MUTED },
  cardDesc: { fontSize: 13, fontFamily: 'Coolvetica-Regular', color: MUTED, marginBottom: 5 },
  cardTimeRow: { flexDirection: 'row', alignItems: 'center', gap: 5, flexWrap: 'wrap' },
  cardTime: { fontSize: 14, fontFamily: 'Coolvetica-Bold', color: DARK },
  ...
  countdownText: { fontSize: 11, fontFamily: 'Coolvetica-Bold', color: DARK },
  pastPill: { backgroundColor: '#F3F4F6', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
  pastPillText: { fontSize: 11, fontFamily: 'Coolvetica-Bold', color: MUTED },
  ...
  importanceBadgeText: { fontSize: 10, fontFamily: 'Coolvetica-Bold', letterSpacing: 0.3 },
  ...
  emptySubtitle: { fontSize: 15, fontFamily: 'Coolvetica-Regular', color: MUTED, textAlign: 'center', paddingHorizontal: 40 },
  ...
  emptyBtnText: { fontSize: 16, fontFamily: 'Coolvetica-Bold', color: DARK },
  ...
  forPatientBanner: { flexDirection: 'row', alignItems: 'center', gap: 7, alignSelf: 'flex-start', backgroundColor: TEAL + '14', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: TEAL + '33', marginBottom: 16 },
...
const modal = StyleSheet.create({
  ...
  stepLabel: { fontSize: 11, fontFamily: 'Coolvetica-Bold', color: MUTED, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 2 },
  ...
  fieldLabel: { fontSize: 11, fontFamily: 'Coolvetica-Bold', color: MUTED, letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 8 },
  inputLarge: { backgroundColor: WHITE, borderWidth: 1.5, borderColor: BORDER, borderRadius: 16, paddingHorizontal: 18, paddingVertical: 16, fontSize: 18, fontFamily: 'Coolvetica-Regular', color: DARK, marginBottom: 16, minHeight: 70 },
  input: { backgroundColor: WHITE, borderWidth: 1.5, borderColor: BORDER, borderRadius: 16, paddingHorizontal: 18, paddingVertical: 14, fontSize: 16, fontFamily: 'Coolvetica-Regular', color: DARK, marginBottom: 16 },
  importanceGrid: { flexDirection: 'row', gap: 10, marginBottom: 8 },
  importanceCard: { flex: 1, borderRadius: 20, borderWidth: 1.5, padding: 14, alignItems: 'center', gap: 8, position: 'relative' },
  importanceCheck: { position: 'absolute', top: 10, right: 10, width: 18, height: 18, borderRadius: 9, justifyContent: 'center', alignItems: 'center' },
  importanceIconWrap: { width: 52, height: 52, borderRadius: 16, justifyContent: 'center', alignItems: 'center' },
  importanceLabel: { fontSize: 15, fontFamily: 'Coolvetica-Bold', letterSpacing: 0.3 },
  importanceDesc: { fontSize: 10, fontFamily: 'Coolvetica-Regular', color: MUTED, textAlign: 'center', lineHeight: 14 },
  ...
  selectedDateText: { fontSize: 15, fontFamily: 'Coolvetica-Bold', color: DARK },
  ...
  summaryTitle: { fontSize: 16, fontFamily: 'Coolvetica-Bold', color: DARK, marginBottom: 4 },
  summaryMeta: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  summaryDate: { fontSize: 13, fontFamily: 'Coolvetica-Regular', color: MUTED },
  summaryImportancePill: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8, borderWidth: 1 },
  summaryImportanceText: { fontSize: 11, fontFamily: 'Coolvetica-Bold' },
  ...
  backBtnText: { fontSize: 15, fontFamily: 'Coolvetica-Bold', color: DARK },
  ...
  nextBtnText: { fontSize: 16, fontFamily: 'Coolvetica-Bold', color: DARK },
...
const cal = StyleSheet.create({
  ...
  monthLabel: { fontSize: 18, fontFamily: 'Coolvetica-Bold', color: DARK },
  dayNamesRow: { flexDirection: 'row', marginBottom: 6 },
  dayName: { flex: 1, textAlign: 'center', fontSize: 12, fontFamily: 'Coolvetica-Bold', color: MUTED },
  ...
  cellText: { fontSize: 15, fontFamily: 'Coolvetica-Regular', color: DARK },
  cellTextToday: { fontFamily: 'Coolvetica-Bold', color: DARK, textDecorationLine: 'underline' },
  cellTextSelected: { color: YELLOW, fontFamily: 'Coolvetica-Bold' },
...
const tp = StyleSheet.create({
  ...
  column: { width: 72 },
  colLabel: { fontSize: 11, fontFamily: 'Coolvetica-Bold', color: MUTED, letterSpacing: 1.2, textTransform: 'uppercase', textAlign: 'center', marginBottom: 8 },
  scroll: { height: 160 },
  item: { paddingVertical: 10, borderRadius: 10, alignItems: 'center' },
  itemSelected: { backgroundColor: DARK },
  itemText: { fontSize: 20, fontFamily: 'Coolvetica-Regular', color: DARK },
  itemTextSelected: { color: YELLOW, fontFamily: 'Coolvetica-Bold' },
  colon: { fontSize: 28, fontFamily: 'Coolvetica-Bold', color: DARK, alignSelf: 'center', marginTop: 20 },
  preview: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  previewText: { fontSize: 36, fontFamily: 'Coolvetica-Heavy-Regular', color: DARK },
  previewSub: { fontSize: 13, fontFamily: 'Coolvetica-Regular', color: MUTED, marginTop: 4 },
});